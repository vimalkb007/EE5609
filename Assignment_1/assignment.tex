...
\section{\textbf{Solution}}
	...
	It is given that $\vec{d}\perp\vec{a}$, then their correponding dot product will be 0.\hfill \break
		$\vec{d}$$\cdot$$\vec{a}$ = 0 $\implies$ $\myvec{x\\y\\z}$$\cdot$$\myvec{1\\4\\2}$ = 0 \hfill \break
	...
	$\vec{d}$$\cdot$$\vec{b}$ = 0 $\implies$ $\myvec{x\\y\\z}$$\cdot$$\myvec{3\\-2\\7}$ = 0 
	\begin{equation}\label{eq2}
		3x - 2y + 7z = 0
	\end{equation} 

	Since, it is given that $\vec{d}^T\vec{c} = 15$, we can write it as 
	(\begin{matrix}x & y & z\end{matrix})  $\times$  $\myvec{2\\-1\\4}$ = 15.
    ...
    Applying 
    $R_3 \leftarrow R_3 - \frac{9}{14}R_2, $ 
